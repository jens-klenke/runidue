...
opts_chunk$set(echo = F, fontsize = "\\footnotesize", dev = "tikz", cache = F, message = FALSE, warning = FALSE,
...
plot <- function(..., col = NULL, pch = NULL) {
  graphics::plot(..., bty = "o",
                 col = ifelse(is.null(col), due.col$blue, col),
...
knit_hooks$set(c.par = function(before, options, envir) {
  if (before) {
    ...
    magnify <- approxfun(x, y, yleft = 0.5)
  }
})
